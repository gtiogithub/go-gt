...
func NewMVCCDB(storage cdb.Storage, trieSameKeyCompatibility bool) (*MVCCDB, error) {
	tid := new(int)
...
// Del value
func (db *MVCCDB) Delete(key []byte) error {
	db.mutex.Lock()
	defer db.mutex.Unlock()
...
func (db *MVCCDB) Has(key []byte) (bool, error) {
	...
	return false, cdb.ErrKeyNotFound
}

// ValueSize
func (db *MVCCDB) ValueSize() int {
	return 0
}
