...
func (st *Task) startSyncLoop() {
	for {
		...
		syncTicker := time.NewTicker(GetChunkDataTimeout * time.Second)
	...
	SyncStep1:
		...
		getChunkTimeoutTicker := time.NewTicker(GetChunkDataTimeout * time.Second)
...
func (st *Task) chunkHeadersRequest() {
	...
	if err != nil {
		...
		}).Error("Failed to serialize sync message")
		return
	}
...
func (st *Task) processChunkHeaders(message net.Message) {
	...
	if st.chainSyncPeers == nil {
		...
		}).Error("Invalid ChainChunkHeaders message source peer, chinSyncPeers is NIL.")
	...
	if isValidSourcePeer == false {
		logging.VLog().WithFields(logrus.Fields{
			"err": ErrInvalidChunkHeaderSourcePeer,
			"pid": message.MessageFrom(),
		}).Error("Invalid ChainChunkHeaders message source peer.")
		st.netService.ClosePeer(message.MessageFrom(), ErrInvalidChunkHeaderSourcePeer)
		return
	}
	...
	if err := proto.Unmarshal(message.Data(), chunkHeaders); err != nil {
		logging.VLog().WithFields(logrus.Fields{
			"err": err,
			"pid": message.MessageFrom(),
		}).Error("Invalid ChainChunkHeaders message data.")
	...
	if ok, err := verifyChunkHeaders(chunkHeaders); ok == false {
		logging.VLog().WithFields(logrus.Fields{
			"err": err,
			"pid": message.MessageFrom(),
		}).Error("Wrong ChainChunkHeaders message data.")
...
func (st *Task) processChunkData(message net.Message) {
	...
	if st.maxConsistentChunkHeaders == nil || st.maxConsistentChunkHeaders.ChunkHeaders == nil {
		logging.VLog().WithFields(logrus.Fields{
			"pid": message.MessageFrom(),
		}).Error("Invalid ChainChunkData message data.")
	...
	if chunkDataIndex < 0 {
		logging.VLog().WithFields(logrus.Fields{
			"pid": message.MessageFrom(),
		}).Error("Wrong ChainChunkData message data.")
	...
	if st.chainChunkDataStatus[chunkDataIndex] == chunkDataStatusFinished {
		logging.VLog().WithFields(logrus.Fields{
			"pid": message.MessageFrom(),
		}).Error("Duplicated ChainChunkData message data.")
		return
	}
	...
	if ok, err := verifyChunkData(chunkHeader, chunkData); ok == false {
		logging.VLog().WithFields(logrus.Fields{
			"err": err,
			"pid": message.MessageFrom(),
		}).Error("Wrong ChainChunkData message data, retry.")
...
func (st *Task) chunkDataRequest(chunkHeaderIndex int) {
	...
	_ = st.netService.SendMessageToPeer(net.ChunkDataRequest, data, net.MessagePriorityLow, peers[idx])
