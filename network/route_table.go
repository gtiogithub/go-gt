...
func (table *RouteTable) Start() {
	logging.CLog().Info("Starting GtService RouteTable Sync...")
...
func (table *RouteTable) syncLoop() {
	...
	logging.CLog().Info("Started GtService RouteTable Sync.")
	...
	for {
		select {
		case <-table.quitCh:
			logging.CLog().Info("Stopped GtService RouteTable Sync.")
			return
...
func (table *RouteTable) Stop() {
	logging.CLog().Info("Stopping GtService RouteTable Sync...")
