...
import (
	...
	netpb "gt.pro/gtio/go-gt/network/pb"
...
type Stream struct {
	...
	highPriorityMessageChan   chan *GtMessage
	normalPriorityMessageChan chan *GtMessage
	lowPriorityMessageChan    chan *GtMessage
...
func newStreamInstance(pid peer.ID, addr ma.Multiaddr, stream network.Stream, node *Node) *Stream {
	return &Stream{
		...
		highPriorityMessageChan:   make(chan *GtMessage, 2*1024),
		normalPriorityMessageChan: make(chan *GtMessage, 2*1024),
		lowPriorityMessageChan:    make(chan *GtMessage, 2*1024),
...
func (s *Stream) handleMessage(message *GtMessage) error {
...
func (s *Stream) onHello(message *GtMessage) error {
...
func (s *Stream) onOk(message *GtMessage) error {
...
func (s *Stream) onBye(message *GtMessage) error {
	logging.VLog().WithFields(logrus.Fields{
		"stream": s.String(),
...
func (s *Stream) onSyncRoute(message *GtMessage) error {
	return s.RouteTable()
}
...
func (s *Stream) onRouteTable(message *GtMessage) error {
	data, err := s.getData(message)
	if err != nil {
		return err
	}
...
func (s *Stream) WriteMessage(messageName string, data []byte, reservedClientFlag byte) error {
	...
	message, err := NewGtMessage(s.node.config.NetworkID, s.node.config.ChainID, reserved, CurrentVersion, messageName, data)
	if err != nil {
		return err
	}

	return s.WriteGtMessage(message)
}

// WriteGtMessage write gt msg in the stream
func (s *Stream) WriteGtMessage(message *GtMessage) error {
	// metrics.
...
func (s *Stream) SendMessage(messageName string, data []byte, priority int) error {
	message, err := NewGtMessage(s.node.config.NetworkID, s.node.config.ChainID, s.reservedFlag, CurrentVersion, messageName, data)
	if err != nil {
		return err
	}
	...
	for {
		...
		for {
			if message == nil {
				...
				if len(messageBuffer) < GtMessageHeaderLength {
					// continue reading.
					break
				}

				message, err = ParseGtMessage(s.node.config.NetworkID, messageBuffer)
				if err != nil {
					s.Bye()
					return
				}
				...
				messageBuffer = messageBuffer[GtMessageHeaderLength:]
			}
...
func (s *Stream) getData(message *GtMessage) ([]byte, error) {
	var data []byte
